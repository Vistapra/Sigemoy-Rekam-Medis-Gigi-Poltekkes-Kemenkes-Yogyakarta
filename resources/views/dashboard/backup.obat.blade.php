                                        ...
                                        @if ($query->permintaanObat()->count() > 0)
                                            @foreach ($query->permintaanObat() as $item)
                                                <div class="d-flex pb-3 border-bottom mb-3 align-items-end">
                                                    <div class="mr-auto">
                                                        <p class="text-black font-w600 mb-2"><a
                                                                href="{{ Route('rekam.detail', $item->pasien_id) }}">{{ $item->pasien->nama }}</a>
                                                        </p>
                                                        <ul>
                                                            <li><i class="las la-clock"></i>Time :
                                                                {{ $item->updated_at->diffForHumans() }}</li>
                                                            <li><i class="las la-user"></i>Resep Obat :
                                                                {!! $item->resep_obat !!}</li>
                                                        </ul>
                                                    </div>
                                                    <a href="{{ Route('obat.pengeluaran', $item->id) }}"
                                                        class="btn-rounded btn-primary btn-xs"><i class="fa fa-user-md"></i>
                                                        Berikan Obat</a>
                                                </div>
                                            @endforeach
                                        @endif
