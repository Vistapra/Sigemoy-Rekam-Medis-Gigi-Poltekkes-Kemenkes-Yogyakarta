...
@section('content')
    <div class="form-head d-flex align-items-center mb-sm-4 mb-3">
        <div class="mr-auto">
            ...
            <p class="mb-0">Dr. {{ auth()->user()->name }}</p>
        </div>
    </div>
    <div class="row">
                ...
                <div class="card-body">
                    <div class="media align-items-center">
                        <div class="media-body mr-3">
                            <h2 class="fs-34 text-black font-w600">{{ $query->pasienHariini() }}</h2>
                ...
                <div class="card-body">
                    <form action="{{ route('dashboard') }}" method="GET" class="mb-3">
                        <div class="input-group">
                            <input type="text" class="form-control" placeholder="Cari nama pasien..." name="search" value="{{ $search ?? '' }}">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @forelse ($rekamMedis as $item)
                                    <tr>
                                        <td><a href="{{ route('rekam.detail', $item->pasien_id) }}" class="text-primary">{{ $item->pasien->nama }}</a></td>
                                        <td>{{ $item->created_at->diffForHumans() }}</td>
                                        <td>{{ $item->keluhan }}</td>
                                        <td>{{ $item->user->name }}</td>
                                        <td>
                                            <a href="{{ route('rekam.detail', $item->pasien_id) }}" class="btn btn-sm btn-outline-primary">
                    ...
                    @if ($rekamMedis->hasPages())
                        <div class="pagination-wrapper py-4">
                            <ul class="pagination justify-content-center">
                                ...
                                @else
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $rekamMedis->previousPageUrl() }}" rel="prev" aria-label="@lang('pagination.previous')">&lsaquo;</a>
                                    </li>
                                @endif

                                {{-- Pagination Elements --}}
                                @foreach ($rekamMedis->getUrlRange(max($rekamMedis->currentPage() - 1, 1), min($rekamMedis->currentPage() + 1, $rekamMedis->lastPage())) as $page => $url)
                                ...
                                @if ($rekamMedis->hasMorePages())
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $rekamMedis->nextPageUrl() }}" rel="next" aria-label="@lang('pagination.next')">&rsaquo;</a>
                                    </li>
                                @else
                                    ...
                                    <div class="d-flex flex-wrap align-items-center px-4 bg-light">
                                        <div class="mr-auto d-flex align-items-center py-3">
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    class="fs-26 text-primary font-w600">{{ $query->pasienHariini() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="d-flex flex-wrap align-items-center px-4 bg-light">
                                        <div class="mr-auto d-flex align-items-center py-3">
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    class="fs-26 text-primary font-w600">{{ $query->perikaBulanini() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="d-flex flex-wrap align-items-center px-4 bg-light">
                                        <div class="mr-auto d-flex align-items-center py-3">
                                            ...
                                            <div>
                                                ...
                                                <span
                                                    class="fs-26 text-primary font-w600">{{ $query->perikaTahunini() }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="tab-content">
                                <div class="tab-pane fade show active" id="MonthlyDiagnosa" role="tabpanel">
                                    @forelse($query->diagnosaBulanan() as $item)
                                        <div class="d-flex mb-3 align-items-center">
                                            <span class="mr-2 rounded-circle bg-primary"
                                                style="width:20px;height:20px;"></span>
                                            <span class="fs-14 text-black mr-auto d-inline-block text-truncate"
                                                style="max-width: 180px;">{{ $item->diagnosa }} -
                                                {{ $item->name_id }}</span>
                                            <span class="fs-14 text-primary font-w600">{{ $item->total }}</span>
                                        </div>
                                    @empty
                                ...
                                <div class="tab-pane fade" id="YearlyDiagnosa" role="tabpanel">
                                    @forelse($query->diagnosaYearly() as $item)
                                        <div class="d-flex mb-3 align-items-center">
                                            <span class="mr-2 rounded-circle bg-primary"
                                                style="width:20px;height:20px;"></span>
                                            <span class="fs-14 text-black mr-auto d-inline-block text-truncate"
